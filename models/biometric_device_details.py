    ...
    def action_download_attendance(self):
        ...
        for info in self:
            ...
            if conn:
                ...
                if attendance:
                    # Process punches in chronological order. This matters
                    # because our devices don't tag punches as Check In /
                    # Check Out, so we determine that automatically based
                    # on order (see below) - if they're processed out of
                    # order, punches could be wrongly paired.
                            ...
                            if uid.user_id == each.user_id:
                                ...
                                if get_user_id:
                                    ...
                                    if not duplicate_atten_ids:
                                        ...
                                        # Some ZKTeco models (including the
                                        # K-60) don't reliably report
                                        # each.punch as 0/1 (Check In/Check
                                        # Out) - the device just logs a
                                        # verified scan with no in/out
                                        # label. So instead of trusting
                                        # each.punch, we infer it: if the
                                        # employee has no open attendance
                                        # (no check_out=False record), this
                                        # scan is a Check In. If they do,
                                        # this scan closes it as a Check Out.
                                        ...
                                        if not att_var:
                                            ...
                                            # Open attendance exists -> this
                                            # scan is the Check Out, as long
                                            # as it's after the check-in
                                            # time (guards against
                                            # out-of-order/duplicate scans).
